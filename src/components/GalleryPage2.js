...
function GalleryPage2() {
  const startImage = 57;  // Starting image number for this gallery
  const endImage = 106;  // Ending image number
  const images = [];

  for (let i = startImage; i <= endImage; i++) {
    images.push(
      <img key={i} src={`${process.env.PUBLIC_URL}/img/img2/buster${i}.webp`} alt={`Buster ${i}`} loading="lazy" />
    );
  }
